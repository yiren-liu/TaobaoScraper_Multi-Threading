...
def main(work, key, num, mkname, start_price, end_price, page_num):
    ...
    try:
        ...
        print (e)
    for i in range(20):
        print ("当前正在采集第 ", i + 1, " 个,第", str(page_num), ' 页','关键词',str(key))
        try:
...
def multi(i,date_list,page_num,key):
...
if __name__ == '__main__':
    ...
    if (key == '' or name == '' or num == ''):
        ...
        page_num = 20
        for i in name:
            t =threading.Thread(target=multi,args=(i,date_list,page_num,key,))
            t.start()
